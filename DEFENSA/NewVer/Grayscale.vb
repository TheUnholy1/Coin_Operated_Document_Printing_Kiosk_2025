...
Public Class Grayscale
    ...
    Private LetterSizeCostPerPage As Integer = Execute.LetterPriceG
    Private LegalSizeCostPerPage As Integer = Execute.LegalPriceG
    ...
    Private Async Sub Grayscale_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not String.IsNullOrEmpty(pdfFilePath) Then
            ...
            ' Validate page range and disable print button if total pages to print exceeed 60
            UpdatePreview()
        End If
        ...
        ComputePayment()
        ResetChangetoZero()
        'LetterSizeCostPerPage = Integer.Parse(Execute.LetterPriceG)
    End Sub
    ...
    Private Sub UpdatePreview()
        Try
            ...
            If fromPage > toPage Then
                Dim showrangeForm As New Range()
                showrangeForm.ShowDialog()
                            ...
                            Await Task.Run(Sub()
                                               Try
                                                   ...
                                                   If processSuccess Then
                                                       ...
                                                       '     Dim changeToDeduct As Integer
                                                       '   Integer.TryParse(lblChange.Text.Trim(), changeToDeduct)
    ...
    Private Sub PrintImage(imagePath As String, printerName As String, isLetterSize As Boolean)
        ...
        Try
            ...
            InsertTransaction(1, pdfFileName, totalPagesToPrint, "Grayscale", totalPayment, sukli, "Pending")
            printDoc.Print()

        Catch ex As Exception
            processSuccess = False
            ' In case of error during the printing process
    ...
    Private Sub Grayscale_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        If SerialPort2.IsOpen Then
            SerialPort2.Close()
        End If
